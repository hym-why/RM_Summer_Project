...
当前为 2026-09-05 实车烧录验证的 PB13/PB12 PID 循迹版：上电后保持停止，按一次扩展板按键开始循迹，再按一次停止。
PID 基准速度为 `440`，参数为 `Kp=140.0`、`Ki=0.0`、`Kd=1.2`，PID 输出限幅为 `360`。
左探头接 `PB13`，右探头接 `PB12`，检测到黑线为高电平。检测到单侧黑线时使用 PID 差速修正；两侧都离开黑线时，根据最后一次有效误差让内侧轮停止、外侧轮以 `650` 搜索黑线。
PID 模式下数码管显示 `0/1/2` 分别表示左偏、居中、右偏，显示 `8` 表示丢线搜索。

### 直角弯控制规则

| 传感器状态 | 数码管 | 控制动作 |
| --- | ---: | --- |
| 只有左侧检测到黑线 | `0` | PID 向左修正 |
| 两侧均检测到黑线 | `1` | 左右轮保持 `440` 基准速度 |
| 只有右侧检测到黑线 | `2` | PID 向右修正 |
| 两侧均未检测到黑线 | `8` | 按最后有效方向执行 `0/650` 或 `650/0` 搜索 |
...
- [PB13/PB12 PID 调参版实车 Demo](documents/demo/pid_line_follow_tuned_demo.mp4)
