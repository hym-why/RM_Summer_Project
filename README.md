...
当前直线参数为左右轮 `440/440`；左转为 `0/560`，右转为 `560/0`。
转弯时短暂丢线会保持原方向，持续丢线超过 900 ms 后停车保护。
当前实车左右传感器接线与逻辑命名相反，因此 `LINE_SENSOR_SWAP` 设为 `1`；若以后交换了 PA11/PA12 接线，将它改回 `0`。
